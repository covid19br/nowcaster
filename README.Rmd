...
```
This element it is the counts of cases by the amount of delay. It is known as the delay triangle, if we table it we can see  how is the pattern of the delay for the cases.  

```{r delay-triangle}
delay_triangle<-table(nowcasting_bh_no_age$dados$Delay, nowcasting_bh_no_age$dados$date_onset,dnn = list("Delay", "Date of Onset"))
head(delay_triangle, 10)
```
