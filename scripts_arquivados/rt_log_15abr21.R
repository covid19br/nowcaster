...
q <- paste("G:/CCD/CVE/RESPIRATORIAS/Coronavírus/Bancos_SRAG/", p4 ,"RT_log/",sep="")

dados <- as.data.frame(read.delim(paste0(p,"base_srag_boletim.csv"), ';'))
...
for (i in seq(1:length(agregado))) {
  ...
  for (j in seq(1:lengths(ids_agregados))) {
    ...
    if (count(nivel_agregado1 %>% filter(nivel_agregado1$I>0))>minimo_dias) {
      ...
      png(
        file = paste(q, 'Rt_Corr-', agregado[i], "-", ids_agregados[[1]][j], ".png", sep = ""),
        width = 1800,
        height = 2200,
        res = 300
      )
