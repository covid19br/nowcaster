---
title: "Nowcaster"
output: github_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
	echo = TRUE,
	message = FALSE,
	warning = FALSE
)
library(badger)
```


<a href='https://github.com/covid19br/nowcaster'><img src='man/figures/nowcaster.png' align="right" width="140" /></a> <a href='https://github.com/covid19br/nowcaster'><img src='man/figures/nowcaster_rev.png' align="right" width="140" /></a>

<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/nowcaster)](https://CRAN.R-project.org/package=nowcaster) 
[![CRAN checks](https://badges.cranchecks.info/summary/nowcaster.svg)](https://cran.r-project.org/web/checks/check_results_nowcaster.html)
[![CRAN downloads](https://cranlogs.r-pkg.org/badges/nowcaster)](https://CRAN.R-project.org/package=nowcaster) 
[![Lifecyle: Stable](https://img.shields.io/badge/lifecycle-stable-steelblue.svg)](https://lifecycle.r-lib.org/articles/stages.html#stable)
[![License: GPL (>= 3)](https://img.shields.io/badge/license-GPL (>= 3)-blue.svg)](https://cran.r-project.org/web/licenses/GPL (>= 3))
<!-- badges: end -->

`nowcaster`: Statistical Models for Notification Delay Correction of Epidemiological Data. The package wraps statistical models to estimate not yet reported data using empirical delay distribution either from individual-level data or from the differences of aggregated time series of cases.
`nowcaster` it was built for and during epidemiological emergency use, it was constructed for the Brazilian Severe Acute Respiratory Illness (SARI)
surveillance system (SIVEP-Gripe), at the time of Covid-19 pandemic. 

Every single system of notification has an intrinsic delay between the `date of onset` of the event and the `date of report`. 
`nowcaster` can estimate how many counts of any epidemiological data of interest
(*i.e.*, daily cases and deaths counts) by fitting a negative binomial
model to the time steps of delay between onset date of the event,
(*i.e.*, date of first symptoms for cases or date of occurrence of
death) and the date of report (*i.e.*, date of notification of the case
or death).

# Installing

You can install direct from CRAN:
```{r CRAN install, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
install.packages("nowcaster")
```

Or you can install the developing version directly from GitHub: 

```{r Installing, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
devtools::install_github("https://github.com/covid19br/nowcaster")
```
If you have any problem installing, please refer to next section on the dependencies of the package.

# Dependencies

`nowcaster` is based on the [`R-INLA`](https://www.r-inla.org/download-install) and [`mgcv`](https://cran.r-project.org/web/packages/mgcv/index.html). 
Please make sure you have stable version of those packages to be able to fully work with `nowcaster`

Installing `INLA`:
```{r install INLA, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
install.packages("INLA", repos=c(getOption("repos"), INLA="https://inla.r-inla-download.org/R/stable"), dep=TRUE)
```

Installing `mgcv`:
```{r install mgcv, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
install.packages("mgcv")
```

# Similar Initiatives

There are other alternative packages, that can produce nowcasting estimation, here it is some options:

* [Epinowcast](https://package.epinowcast.org/)
* [Surveillance](https://surveillance.r-forge.r-project.org/pkgdown/index.html)
